...
const errorsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/errors/:id', async (request, reply) => {
    try {
      ...
      fastify.log.error('Error fetching error log:', error);
      return reply.status(500).send({
        success: false,
  ...
  fastify.patch('/errors/:id/resolve', async (request, reply) => {
    try {
      ...
      fastify.log.error('Error resolving error log:', error);
      return reply.status(500).send({
        success: false,
  ...
  fastify.get('/errors/stats/dashboard', async (request, reply) => {
    try {
      ...
      fastify.log.error('Error fetching error stats:', error);
      return reply.status(500).send({
        success: false,
  ...
  fastify.post('/errors/log', async (request, reply) => {
    try {
      ...
      fastify.log.error('Error logging error:', error);
      return reply.status(500).send({
        success: false,
